    ...
    //
    // 전체 시나리오 : https://vintagetalk.co.kr/product/detail.html?product_no=583493&cate_no=1&display_group=2
    ...
    function get_time() {//시간측정 함수
      ...
      // @잠시 주석처리
      // $result = shell_exec("python3 py_mysql.py");
      // // 크롤링 후 db에 저장하는 프로그램을 발동시키고
      // if(strlen($result) == 5){
      //   // 정상적으로 완료될시

        // 1-1. new_products 테이블에서
        //    쇼핑몰이름, 이미지주소, 상품명, 가격, 개수 를 가져오게한다.
        // $query = "SELECT mall_name, img_src, prodname, price, count, href FROM new_products LIMIT $startIndex, $endIndex";
        // 이건 몰 순번 반영 안시킨 쿼리
        $query = "SELECT mol.mall_name, nps.img_src, nps.prodname, nps.price, nps.count, nps.href FROM new_products AS nps JOIN mall_order_list AS mol ON mol.mall_name = nps.mall_name ORDER BY mol.mall_order LIMIT $startIndex, $endIndex";
        // 이건 몰 순선 반영시킨 쿼리
        // # join에 대한 사용법 블로그
        //https://futurists.tistory.com/17
        // $query = "SELECT mall_name, img_src, prodname, price, count, href FROM new_products LIMIT 51, 50";
