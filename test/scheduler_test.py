...
def get_latest_prod():
    ...
    sql = 'SELECT mall_name, latest_product_name, createdDate FROM yj_mall_update_record WHERE mall_name = "realv" ORDER BY createdDate DESC LIMIT 1'
    ...
    try:
        ...
        print("리얼브이 크롤링중 request를 보내는 코드에서 에러 발생(return로 메서드를 종료시켜 다음 반복문이 실행되게함)")
        print(e)
        return
    ...
    anounce_message = 'response.status_code 입니다. = '+str(statuseCode)+' ('+nowDatetime+')'
    # 텔레그램 봇 통해서 메시지 보내기
    ...
    for value in selectedList:
        ...
        latest_product_name = atag[3].text.strip()
    ...
    if before_latestProd_name == latest_product_name:
        ...
        now = datetime.now()
        nowDatetime = now.strftime('%Y-%m-%d %H:%M:%S')
        anounce_message = 'learv[리얼브이] 상품 업데이트 되었습니다 (- -) (_ _) (- -). ('+nowDatetime+')'
        # 텔레그램 봇 통해서 메시지 보내기
        teleurl = "https://api.telegram.org/bot935475572:AAFnM0-sOYGhaUNaMbMD68dEeF7v24tCBu4/sendMessage"
        params = {'chat_id':'972295152', 'text':anounce_message}
        res = requests.get(teleurl, params=params)
...
schedule.every(10).seconds.do(get_latest_prod)
...
now = datetime.now()
nowDatetime = now.strftime('%Y-%m-%d %H:%M:%S')
anounce_message = 'learv[리얼브이] 체크 시작합니다. ('+nowDatetime+')'
# 텔레그램 봇 통해서 메시지 보내기
teleurl = "https://api.telegram.org/bot935475572:AAFnM0-sOYGhaUNaMbMD68dEeF7v24tCBu4/sendMessage"
params = {'chat_id':'972295152', 'text':anounce_message}
res = requests.get(teleurl, params=params)
...
now = datetime.now()
nowDatetime = now.strftime('%Y-%m-%d %H:%M:%S')
anounce_message = 'learv[리얼브이] 체크 코드 종료합니다. ('+nowDatetime+'):while문 out'
# 텔레그램 봇 통해서 메시지 보내기
teleurl = "https://api.telegram.org/bot935475572:AAFnM0-sOYGhaUNaMbMD68dEeF7v24tCBu4/sendMessage"
params = {'chat_id':'972295152', 'text':anounce_message}
res = requests.get(teleurl, params=params)
